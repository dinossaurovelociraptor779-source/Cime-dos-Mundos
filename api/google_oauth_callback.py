...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        self._reply(200,{"ok":True,"route":"/api/google_oauth_callback","method":"GET","service":"Cime dos Mundos 5.0"})
    ...
    def do_POST(self):
        try:
            ...
            if not verify_state(state,cookie_value(self,"cime_google_state")):
            ...
            data=__import__("urllib.parse").parse.urlencode({
                "code":code,
                "client_id":CLIENT_ID,
            ...
            req=Request("https://oauth2.googleapis.com/token",data=data,headers={"Content-Type":"application/x-www-form-urlencoded","User-Agent":"CimeDosMundos/5.0"})
            ...
            token=server_v5.token_make(uid,self.headers.get("User-Agent",""))
            raw=json.dumps({"ok":True,"token":token,"user":{"id":uid,"email":email,"display_name":name,"role":role,"provider":"google"}},ensure_ascii=False,separators=(",",":")).encode()
            self.send_response(200)
            self.send_header("Content-Type","application/json; charset=utf-8")
            self.send_header("Cache-Control","no-store")
            self.send_header("Set-Cookie","cime_google_state=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Lax")
            self.send_header("Content-Length",str(len(raw)))
            self.end_headers()
            self.wfile.write(raw)
